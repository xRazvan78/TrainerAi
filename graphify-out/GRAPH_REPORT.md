...
## Corpus Check
- 114 files · ~61,872 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1279 nodes · 1385 edges · 110 communities (73 shown, 37 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 84 edges (avg confidence: 0.81)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `845a8879`
...
6. `Phase G.4: Verification & Acceptance` - 11 edges
7. `embed_text()` - 10 edges
8. `ingest_video()` - 10 edges
9. `Sub-phase C.1 — Config + Dependencies` - 10 edges
10. `Sub-phase C.2 — LLM Service (Qwen SSE Streaming Client)` - 10 edges
...
Cohesion: 0.05
Nodes (26): BaseModel, PerceptionElement, PerceptionStatePersistedResponse, PerceptionStateRequest, EmbeddingCreate, EmbeddingUpdate, SessionCreate, SessionUpdate (+18 more)
...
Cohesion: 0.06
Nodes (28): CommandRequest, ahash(), bgra_to_rgba(), capture_window_frame(), CapturedFrame, encode_jpeg(), find_autocad_hwnd(), finish_frame() (+20 more)
...
Cohesion: 0.09
Nodes (22): Acceptance Criteria (whole-phase recap), code:python (@pytest.fixture(autouse=True)), code:powershell (pytest --collect-only -q tests/ | Select-String -Pattern "ac), code:powershell (cd trainerAI_backend), code:powershell (docker compose up -d), code:powershell (cd trainerAI_overlay), code:powershell (Invoke-RestMethod "http://localhost:8000/db/perception_state), code:powershell (Invoke-RestMethod "http://localhost:8000/db/sessions?session) (+14 more)
...
Cohesion: 0.16
Nodes (18): ContextPacketFoundation, SessionSnapshot, build_context_packet_foundation(), _build_next_command_sequence(), _ensure_session_exists(), _extract_active_tool(), _extract_active_tool_from_perception(), _normalize_command_sequence() (+10 more)
...
Cohesion: 0.1
Nodes (19): code:python (import asyncio), code:python (if payload.frame_b64 and not payload.elements:), code:python (def _extract_active_tool_from_perception(perception_state: d), code:python (active_tool = _extract_active_tool(command_text)), code:powershell (cd trainerAI_backend), code:powershell (pytest tests/test_session_state_service.py -v), Edge Cases & Risks, File & Directory Changes (+11 more)
...
Cohesion: 0.14
Nodes (9): CommandAcceptedResponse, command_endpoint(), process_command_placeholder(), _utc_now_iso8601(), run_week2_command_pipeline(), safe_run_week2_command_pipeline(), broadcast_done(), broadcast_token() (+1 more)
...
Cohesion: 0.12
Nodes (16): code:block1 (ultralytics>=8.3.0), code:powershell (cd trainerAI_backend), code:powershell (python -c "from ultralytics import YOLO; import easyocr; fro), code:powershell (New-Item -ItemType Directory -Force -Path trainerAI_backend\), code:block5 (trainerAI_backend/app/models_weights/*.pt), code:powershell (pytest tests/ -q), Edge Cases & Risks, File & Directory Changes (+8 more)

### Community 38 - "Community 38"
Cohesion: 0.12
Nodes (16): code:python (# trainerAI_backend/app/services/perception_service.py), code:powershell (python -c "from app.services.perception_service import analy), code:powershell (python -c "), Design decisions, Edge Cases & Risks, File & Directory Changes, Goals, Implementation Steps (+8 more)
...
Cohesion: 0.17
Nodes (11): code:block1 (G.1 (deps + weights dir) ──► G.2 (perception_service.py) ──►), Context, Definition of done for the whole phase, Out of scope for Phase G, Phase dependency graph, Phase G — AutoCAD-Specific Perception (Execution Plan), Reused existing code, Sub-phase index (+3 more)
...
Cohesion: 0.43
Nodes (4): _make_black_jpeg_b64(), test_analyse_frame_heuristic_extracts_command_line(), test_analyse_frame_no_weights_skips_yolo_silently(), test_analyse_frame_yolo_overrides_heuristic_command_line()
...
## Knowledge Gaps
- **593 isolated node(s):** `WebSocket endpoint for streaming AI guidance to the overlay client. One connect`, `Real semantic embedding service using sentence-transformers all-MiniLM-L6-v2. P`, `Load the model once and keep it in memory for the process lifetime.`, `Embed a single string into a 384-dimensional float vector.     Thread-safe; mod`, `Embed many strings in one model call (~10x faster than calling embed_text in a l` (+588 more)
...
- **Why does `create_pool()` connect `Community 3` to `Community 21`?**
  _High betweenness centrality (0.019) - this node is a cross-community bridge._
- **Why does `ingest_video()` connect `Community 3` to `Community 34`, `Community 4`, `Community 15`?**
  _High betweenness centrality (0.017) - this node is a cross-community bridge._
- **Why does `get_settings()` connect `Community 21` to `Community 3`?**
